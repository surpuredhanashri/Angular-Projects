...
import { CONTROLS, BOARD_SIZE, GAME_MODES } from '../utility/snake.constant';
...
export class SnakeComponent {
  private gameName: string = "snakeGame"
  ...
  public best_score = this.bestScoreService.retrieve(this.gameName);
  ...
  setImages(col: number, row: number): string {

    if (this.isGameOver) {
      return 'gameOver';
    } else if (this.fruit.x === row && this.fruit.y === col) {
      return 'rabbit';
    } else if (this.snake.parts[this.snake.parts.length-1].x === row && this.snake.parts[this.snake.parts.length-1].y === col) {
      return 'snakeTail';
  ...
  gameOver(): void {
    ...
    if (this.score > this.best_score) {
      this.bestScoreService.store(this.gameName, this.score);
